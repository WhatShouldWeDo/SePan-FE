...
import에 `getCategoryLabel` 추가:
```typescript
import { getCategoryLabel } from "@/features/policy/data/mock-policy";
```

본문에서 필드 참조 변경:
- `{category}` → `{categoryLabel}` (분야 배지 텍스트, getCategoryLabel 헬퍼 사용)
...
- `pledge.category` → `getCategoryLabel(pledge.categoryIds[0])` (카테고리 배지 텍스트, 공유 헬퍼 사용)
					...
					<div className="flex flex-col gap-2">
						<div className="flex items-center gap-2">
							<h2 className="text-heading-3 font-bold text-label-strong">
								{watchedTitle || (pledge ? "공약 제목" : "새 공약 추가하기")}
							</h2>
