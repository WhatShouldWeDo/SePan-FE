...
└── assets/pledges/
    ├── election-icon.png          # 선거 아이콘 (mask-luminance)
...
- **선거 유형 카드 (ElectionTypeCard)**: 3개 카드 그리드, 클릭 시 하위 라우트로 이동
  - 상단: 분홍 배경 + mask-luminance 아이콘
  - 하단: 선거명 + 설명
- **빠른시작 카드 (QuickStartCard)**: 3개 카드 그리드, 최근 검색 선거회차
  - 좌측: 64px 아이콘
  - 우측: 선거회차명 + 지역구 배지 + 정당 배지
- 데이터: 하드코딩 (API 연동 전)
...
- `election-icon.png`: CategoryNav과 동일한 mask-luminance 방식으로 렌더링. `bg-[#b51d52]` 배경색에 maskImage로 적용
- `location-fill.svg`: 지역구 배지 내 위치 아이콘 (`<img>` 태그로 사용)
